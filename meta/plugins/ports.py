...
def portFunc(args: model.TargetArgs):
    ...
    shell_run(
        [
            "cp",
            "-r",
            os.path.join("meta", "build", "cross", "x86_64-pc-wingos", "lib") + "/.",
            SYSROOT_LIB_PATH,
        ]
    )
    ...
    manifest2 = scope.registry.lookup(
        "iol-wingos", type=model.Component, includeProvides=True
    )
    ...
    manifest_libc = scope.registry.lookup(
        "libc-custom", type=model.Component, includeProvides=True
    )
    ...
    manifest_iol = scope.registry.lookup(
        "iol-wingos", type=model.Component, includeProvides=True
    )
    iol_product = builder.build(scope, manifest_iol)[0]
    outfile_iol = builder.outfile(iol_product)

    manifest_liballoc = scope.registry.lookup(
        "liballoc-blanham", type=model.Component, includeProvides=True
    )
    liballoc_product = builder.build(scope, manifest_liballoc)[0]
    outfile_liballoc = builder.outfile(liballoc_product)

    manifest_libcore = scope.registry.lookup(
        "libcore", type=model.Component, includeProvides=True
    )
    libcore_product = builder.build(scope, manifest_libcore)[0]
    outfile_libcore = builder.outfile(libcore_product)

    manifest_libarch = scope.registry.lookup(
        "userspace-arch", type=model.Component, includeProvides=True
    )
    libarch_product = builder.build(scope, manifest_libarch)[0]
    outfile_libarch = builder.outfile(libarch_product)

    shell.cp(outfile_libc, os.path.join(SYSROOT_LIB_PATH, "libc_libc.a"))
    shell.cp(outfile_iol, os.path.join(SYSROOT_LIB_PATH, "libc_iol.a"))
    shell.cp(outfile_liballoc, os.path.join(SYSROOT_LIB_PATH, "libc_liballoc.a"))
    shell.cp(outfile_libcore, os.path.join(SYSROOT_LIB_PATH, "libc_libcore.a"))
    shell.cp(outfile_libarch, os.path.join(SYSROOT_LIB_PATH, "libc_libarch.a"))
    ...
    shell_run(
        [
            os.path.join("meta/build/cross/bin/x86_64-pc-wingos-ar"),
            "crsT",
    ...
    shell_run(
        [
            os.path.join("meta/build/cross/bin/x86_64-pc-wingos-ranlib"),
            os.path.join(SYSROOT_LIB_PATH, "libc.a"),
        ]
    )

    shell.cp(crt0_path, os.path.join(SYSROOT_LIB_PATH, "crt0.o"))
