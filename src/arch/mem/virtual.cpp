...
__attribute__((always_inline)) static inline void set_frame_region(uint64_t i, uint64_t count) {
      i -= BITMAP_BASE;
    size_t f = i + count;
    for (size_t j = i; j < f; j++)
        set_frame_bit( j);
}
__attribute__((always_inline)) static inline void unset_frame_region(uint64_t i, uint64_t count) {
