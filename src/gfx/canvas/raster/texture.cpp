...
void wgfx::RasterCanvas::texturePixelAlignedFlat(TextureCommand const &cmd)
{
    ...
    for (long y = fc::max(cmd.rect.start.y, size.start.y); y < fc::min(cmd.rect.end.y, size.end.y); y++)
    {
        float iy = y - cmd.rect.start.y;

        long v = (iy);
        for (long x = fc::max(cmd.rect.start.x, size.start.x); x < fc::min(cmd.rect.end.x, size.end.x); x++)
        {
