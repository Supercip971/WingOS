...
#define flags fmt::FMT_HEX | fmt::FMT_CYAN | fmt::FMT_PAD_8BYTES | fmt::FMT_PAD_ZERO
    ...
    fmt::log$("creating space");
    VmmSpace result = try$(VmmSpace::create(true));

    fmt::log$("mapping space: {}", ctx->_memory_map_count);
    ...
    for (int i = 0; i < ctx->_memory_map_count; i++)
    {
        ...
        fmt::log$("- mapping[{}]: ({}-{}) -> ({}-{}) (mem)", i, entry_phys.start()._addr | flags, entry_phys.end()._addr | flags,
        ...
        {
            fmt::log$("- mapping[{}]: ({}-{}) -> ({}-{}) (module)", i, entry_phys.start()._addr | flags, entry_phys.end()._addr | flags,
