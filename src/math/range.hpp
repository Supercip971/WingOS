...
namespace math 
{
    ...
    public:
        ...
        template<typename C>
        constexpr Range<C> as() const requires(core::IsConvertibleTo<T,C>) { return Range<C>(C(_start), C(_end)); }
    ...
    {
        { a.start() } -> core::IsIdentityIntegral;
        { a.end() } -> core::IsIdentityIntegral;
        { a.length() } -> core::IsIdentityIntegral;
        { a.contains(a.start()) } -> core::IsIdentityIntegral;
        { a.contains(a) } -> core::IsIdentityIntegral;
        { a.overlaps(a) } -> core::IsIdentityIntegral;
    };
}