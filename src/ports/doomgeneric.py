...
from cutekit import builder, cli, const, jexpr, model, rules, vt100
...
def shell_run(cmd, cwd=None):
    ...
    result = os.system(
        "(cd {} && {})".format(
            cwd if cwd else os.getcwd(), " ".join(cmd) if isinstance(cmd, list) else cmd
        )
    )
...
def port():
    ...
    cross_compiler = os.path.join(
        "meta", "build", "cross", "bin", "x86_64-pc-wingos-gcc"
    )
    ...
    shell_run(
        [
            f"CC={os.path.abspath(cross_compiler)}",
            f"CXX={os.path.abspath(cross_compiler).replace('gcc', 'g++')}",
            f"LSCRIPT={os.path.abspath(os.path.join('meta', 'targets', 'wingos-x86_64.ld'))}",
            "make",
            "clean",
            "-j",
        ],
        cwd=os.path.join(MODULE_PATH, "doomgeneric/"),
    )
    ...
    shell_run(
        [
            "cp doomgeneric {}".format(
                os.path.abspath(
                    os.path.join(const.PROJECT_CK_DIR, "wingos-disk/bin/doomgeneric")
                )
            ),
        ],
        cwd=os.path.join(MODULE_PATH, "doomgeneric/"),
