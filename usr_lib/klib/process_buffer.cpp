...
namespace sys
{
    ...
    uint64_t process_buffer::get_length()
    {
        ...
        uint64_t result = sys::service_message("kernel_buffer_service", (uint64_t)&pr, sizeof(pr)).read();
        return result;
    }
    ...
    void process_buffer::out_data(uint8_t *data, unsigned int length)
    {
        ...
        sys::service_message("kernel_buffer_service", (uint64_t)&pr, sizeof(pr)).read();
    }
} // namespace sys
